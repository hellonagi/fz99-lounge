  ...
  const renderParticipantRow = (participant: MergedParticipant & { rank: number }, options?: { showTeamColumn?: boolean }) => {
    ...
    >
      ...
      <td className={cn(
        ...
        !participant.hasSubmitted ? 'text-gray-500' : getPositionColor(participant.rank)
      )}>
      ...
      {/* Machine */}
      <td className="py-2 px-1 md:px-2 text-gray-100 w-8 md:w-auto">
      ...
      {Array.from({ length: isGpMode ? 5 : 3 }, (_, i) => (
        <td key={`r${i + 1}`} className="py-2 px-0.5 md:px-1 text-center text-gray-100 tabular-nums">
      ...
      {!isGpMode && isRated && (
        <td className="py-2 px-1 md:px-2 text-right text-gray-100 tabular-nums">
      ...
      {!isGpMode && isRated && (
        <td className={cn(
          'py-2 px-1 md:px-2 text-right font-medium tabular-nums',
      ...
      {!hideStatus && (
      <td className="py-2 px-1 md:px-2 text-center whitespace-nowrap">
  ...
  const tableHeader = (
    <thead>
      <tr className="border-b border-gray-700 text-gray-400">
        ...
        <th className="text-left py-2 px-1 md:px-2 font-medium w-8 md:w-auto">
          <span className="md:hidden">MC</span>
          <span className="hidden md:inline">Machine</span>
        </th>
        {Array.from({ length: raceColumnCount }, (_, i) => (
          <th key={`rh${i + 1}`} className="text-center py-2 px-0.5 md:px-1 font-medium w-7 md:w-10">R{i + 1}</th>
        ))}
        <th className="text-right py-2 px-1 md:px-2 font-medium">Pts</th>
        {!isGpMode && isRated && (
          <th className="text-right py-2 px-1 md:px-2 font-medium">Rating</th>
        )}
        {!isGpMode && isRated && (
          <th className="text-right py-2 px-1 md:px-2 font-medium">+/-</th>
        )}
        {!hideStatus && <th className="text-center py-2 px-1 md:px-2 font-medium">Status</th>}
      </tr>
    </thead>
  );
      ...
      <div className="overflow-x-auto">
        ...
        <table className="w-full text-xs md:text-sm mb-4">
          <thead>
            <tr className="border-b border-gray-700 text-gray-400">
              <th className="text-left py-2 px-1 md:px-2 font-medium w-0">#</th>
              <th className="text-left py-2 px-1 md:px-2 font-medium">Team</th>
              {Array.from({ length: raceColumnCount }, (_, i) => (
                <th key={`trh${i + 1}`} className="text-center py-2 px-1 font-medium w-8 md:w-10">R{i + 1}</th>
              ))}
              <th className="text-right py-2 px-1 md:px-2 font-medium">Pts</th>
            </tr>
          </thead>
            ...
            {displayTeamScores.map((team) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="py-2 px-1 md:px-2 text-right font-medium text-white">
