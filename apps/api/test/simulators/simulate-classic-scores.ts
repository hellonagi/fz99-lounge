...
class ClassicScoreSimulator {
  ...
  async findInProgressClassicGame() {
    ...
    const submittedUserIds = new Set(
      game.participants.map((p: any) => p.userId),
    );
    ...
    console.log(
      `✅ Found CLASSIC game: Season ${seasonNumber}, Match ${matchNumber}`,
    );
  ...
  generateRaceAssignments(): RaceAssignment[] {
    ...
    const _surviveSlots = getAvailablePositions(3, 1, race3DnfMin - 1).length;
  ...
  async submitScore(user: UserWithToken, raceResults: RaceResult[]) {
    ...
    try {
      ...
      console.log(
        `   📤 ${displayName}: ${resultText} | ${machine}${assistEnabled ? ' +Assist' : ''}`,
      );
  ...
  async run() {
    ...
    try {
      ...
      console.log(`\n✅ Simulation complete!`);
      console.log(
        `📊 View at: http://localhost:3001/matches/classic/${season}/${match}`,
      );
    } catch (error: any) {
